...
int main()
{
    ...
    */
    ...
    // 我们将漫反射强度配置得略高一些；正确的照明条件会因每种照明方法和环境的不同而有所差异。
    // 每种环境和光照类型都需要进行一些调整，以充分利用环境。
    ...
    // 设置灯光衰减系数
    ourShader.setFloat("light.constant",    1.0f); // 常数项
    ourShader.setFloat("light.linear",     0.09f); // 一次项
    ourShader.setFloat("light.quadratic", 0.032f); // 二次项    

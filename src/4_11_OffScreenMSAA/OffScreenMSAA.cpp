...
int main()
{
    ...
    */
    ...
    // 构建一个多重采样的颜色附件材质
    unsigned int textureColorBufferMutiSampled;
    glGenTextures(1, &textureColorBufferMutiSampled);
    glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, textureColorBufferMutiSampled);
    glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, 4, GL_RGB, SCREEN_WIDTH, SCREEN_HEIGHT, GL_TRUE);
    glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, 0);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D_MULTISAMPLE, textureColorBufferMutiSampled, 0);
    // 构建一个多重采样的RBO对象
    unsigned int rbo;
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // 现在将多重采样缓冲区的内容 blit（位块传输）到中间 FBO 的普通颜色缓冲区中。图像存储在 screenTexture 中
